...
| RN021 | **Exploração é detecção-para-prova sob Regras de Engajamento (RoE) de não-dano**: o motor de exploração *executa* o exploit sobre findings já detectados para comprovar impacto real, mas nunca destrói/altera dados, nunca causa DoS, nunca cria persistência, nunca exfiltra dados em massa (amostra limitada a ≤3 linhas), nunca emite payload destrutivo. O piso é aplicado **centralmente** no seam de rede (`apps/scans/exploit/base.py`): método fora da allowlist, payload com token destrutivo (checado após decodificar a URL) ou orçamento de requisições excedido são recusados **antes** de qualquer I/O. Ver `docs/exploitation-engine.md`. |
